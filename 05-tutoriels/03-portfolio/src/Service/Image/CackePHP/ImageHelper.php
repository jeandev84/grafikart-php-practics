...
class ImageHelper
{
    ...
    */
    ...
    public function resizedUrl($file, $width, $height): mixed
    {
          ...
          switch ($info[2]) {
              case IMAGETYPE_GIF: imagegif($image_resized, $this->webRoot . $output, $this->quality); break;
              case IMAGETYPE_JPEG: imagejpeg($image_resized, $this->webRoot . $output, $this->quality); break;
              case IMAGETYPE_PNG: imagepng($image_resized, $this->webRoot. $output, $this->quality); break;
              default: return false;
          }
    }
}